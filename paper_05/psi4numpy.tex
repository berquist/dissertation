...
\onlyifstandalone{\addbibresource{../5746059.bib}}
\onlyifstandalone{\addbibresource{./psi4numpy.bib}}
\onlyifstandalone{\addbibresource{./tutorial.bib}}
...
\begin{document}
\onlyifstandalone{\lstlistoflistings}
...
\begin{lstlisting}[label=snip:np2p42np,caption=Basic example of \pfour{}\(\longleftrightarrow\)\numpy{} interoperability]
import numpy, psi4
...
\begin{lstlisting}[label=snip:numpy_set,caption=Mutating a \pfour{} Matrix directly from a \numpy{} routine]
...
\begin{lstlisting}[label=snip:scf_computation,caption=Initializing a \pfour{} computation from Python]
mol = psi4.geometry("""
O
H 1 0.96
H 1 0.96, 104.5
...
\begin{lstlisting}[label=snip:wfn_data,caption=Obtaining \pfour{} wavefunction data in Python]
...
\begin{lstlisting}[label=snip:mints,caption=Computing atomic orbital-basis integrals from \pfour{} via Python]
...
\begin{lstlisting}[label=snip:eri_trans,caption=Calling \pfour{}'s AO-to-MO transformation from Python]
...
\begin{lstlisting}[label=snip:jk,caption=Building two-electron Fock matrix components using \pfour{} via Python]
...
\begin{lstlisting}[label=snip:aomo,caption=Performing an AO-to-MO transformation using \numpy{}'s \einsum{}]
...
\begin{lstlisting}[label=snip:cc_example,caption=Example of forming a coupled cluster intermediate using \numpy{} and \einsum{}]
...
\IfStandalone{\input{tutorial_redone}}{\input{paper_05/tutorial_redone}}
