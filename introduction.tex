...
\end{equation}
leading to a symmetric matrix \(\mathbf{k}\). In practice, this has implications for computational cost which will be discussed in section~\ref{TODO}.
...
\end{equation}
where \(mu_{i}\) is a component of the dipole (moment), expressed in operator form as
\begin{equation}
...
For example, given the restricted \hf total energy expression,
% \begin{equation}
%   \label{eq:rhf-energy}
%   1 + 1 = 2,
% \end{equation}
\begin{equation}
  \label{eq:yamaguchi-4.1-and-3.115}\tag{Yamaguchi eqs. 4.1, 3.115}
  E = 2 \sum_{i}^{\text{d.o.}} h_{ii} + \sum_{ij}^{\text{d.o.}} \left\{ 2(ii|jj) - (ij|ij) \right\} + \sum_{A>B}^{N} \frac{Z_{A}Z_{B}}{R_{AB}},
\end{equation}
...
% \begin{equation}
%   \label{eq:rhf-gradient}
%   1 + 1 = 2,
% \end{equation}
% \begin{equation}
%   \label{eq:yamaguchi-4.21}
%   \frac{\partial E}{\partial a} = 2 \sum_{i}^{\text{d.o.}} h_{ii}^{a} + \sum_{ij}^{\text{d.o.}} \left\{ 2(ii|jj)^{a} - (ij|ij)^{a} \right\} - 2 \sum_{i}^{\text{d.o.}} S_{ii}^{a} \epsilon_{i},
% \end{equation}
\begin{equation}
  \label{eq:yamaguchi-4.21-and-3.119}\tag{Yamaguchi eqs. 4.21, 3.119}
  \frac{\partial E}{\partial X_{A}} = 2 \sum_{i}^{\text{d.o.}} h_{ii}^{X_{A}} + \sum_{ij}^{\text{d.o.}} \left\{ 2(ii|jj)^{X_{A}} - (ij|ij)^{X_{A}} \right\} - 2 \sum_{i}^{\text{d.o.}} S_{ii}^{X_{A}} \epsilon_{i} - \sum_{B\neq A}^{N} \left(X_{A}-X_{B}\right)\frac{Z_{A}Z_{B}}{R_{AB}^{3}},
\end{equation}
and differentiating that expression once more with respect to another nuclear displacement \(Y_{B}\) is
% \begin{equation}
%   \label{eq:rhf-hessian}
%   1 + 1 = 2.
% \end{equation}
% \begin{equation}
%   \label{eq:yamaguchi-4.54-and-4.55}\tag{Yamaguchi eqs. 4.54 and 4.55}
%   \begin{aligned}
%     \frac{\partial^{2} E}{\partial a \partial b} &= 2 \sum_{i}^{\text{d.o.}} h_{ii}^{ab} + \sum_{ij}^{\text{d.o.}} \left\{ 2(ii|jj)^{ab} - (ij|ij)^{ab} \right\} \\
%     &- 2 \sum_{i}^{\text{d.o.}} S_{ii}^{ab} \epsilon_{i} - 2 \sum_{i}^{\text{d.o.}} \sum_{p}^{\text{all}} \left\{ U_{ip}^{a}U_{ip}^{b} + U_{ip}^{b}U_{ip}^{a} - S_{ip}^{a}S_{ip}^{b} - S_{ip}^{b}S_{ip}^{a} \right\} \epsilon_{i} \\
%     &+ 4 \sum_{p}^{\text{all}} \sum_{i}^{\text{d.o.}} \left( U_{pi}^{b} F_{pi}^{a} + U_{pi}^{a} F_{pi}^{b} \right) + 4 \sum_{p}^{\text{all}} \sum_{i}^{\text{d.o.}} U_{pi}^{a} U_{pi}^{b} \epsilon_{p} \\
%     &+ 4 \sum_{p}^{\text{all}} \sum_{i}^{\text{d.o.}} \sum_{q}^{\text{all}} \sum_{j}^{\text{d.o.}} U_{pi}^{a} U_{qj}^{b} \left\{ 4(pi|qj) - (pq|ij) - (pj|iq) \right\}
%   \end{aligned}
% \end{equation}
\begin{equation}
  \label{eq:yamaguchi-4.54-and-4.55-and-3.127}\tag{Yamaguchi eqs. 4.54, 4.55, 3.127}
  \begin{aligned}
    \frac{\partial^{2} E}{\partial X_{A} \partial Y_{B}} &= 2 \sum_{i}^{\text{d.o.}} h_{ii}^{X_{A}Y_{B}} + \sum_{ij}^{\text{d.o.}} \left\{ 2(ii|jj)^{X_{A}Y_{B}} - (ij|ij)^{X_{A}Y_{B}} \right\} \\
  ...
  \end{aligned}
\end{equation}
where the nucleus-nucleus repulsion energy is included for completeness. In the most general case, there are both derivatives of the AO-basis integrals themselves (such as the nth term in eqn) and of the density matrix (such as the term in), which leads to derivatives of the MO coefficients.
...
\begin{align*}
  \frac{\partial E_{\text{elec}}}{\partial X_{A}} &= 2 \sum_{i}^{\text{d.o.}} h_{ii}^{X_{A}} + \sum_{ij}^{\text{d.o.}} \left[ 2(ii|jj)^{X_{A}} - (ij|ij)^{X_{A}} \right] + 4 \sum_{i}^{\text{d.o.}} \left( -\frac{1}{2} S_{ii}^{X_{A}} \right) \epsilon_{ii} \\
                                                  &= 2 \sum_{i}^{\text{d.o.}} h_{ii}^{X_{A}} + \sum_{ij}^{\text{d.o.}} \left[ 2(ii|jj)^{X_{A}} - (ij|ij)^{X_{A}} \right] - 2 \sum_{i}^{\text{d.o.}} S_{ii}^{X_{A}} \epsilon_{ii}. \label{eq:yamaguchi-4.21}\tag{Yamaguchi eq. 4.21}
\end{align*}
...
p% Similar expressions can be found for the other static molecular property tensors, but they are not reproduced here since only the dynamic versions are required in what follows, and these are derived below. Buckingham (1967, 1978) has given a full account of the static electric molecular property tensors to high order.
