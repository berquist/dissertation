...
The derivatives of the MO coefficients/density matrix will be considered in the context of the \hf equations. Starting from the restricted \hf electronic energy expression,
\begin{equation}
...
\end{equation}
where TODO, the first derivative with respect to a nuclear displacement \(X_{A}\) is\footnote{Additionally, see section C.3 of Szabo and Ostlund\cite{szabo1989modern} and Ref.~\parencite{Pople1979}.}
\begin{equation}
  \label{eq:yamaguchi-4.21}\tag{Yamaguchi eq. 4.21}
  \frac{\partial E}{\partial X_{A}} = 2 \sum_{i}^{\text{d.o.}} h_{ii}^{X_{A}} + \sum_{ij}^{\text{d.o.}} \left\{ 2(ii|jj)^{X_{A}} - (ij|ij)^{X_{A}} \right\} - 2 \sum_{i}^{\text{d.o.}} S_{ii}^{X_{A}} \epsilon_{i},
\end{equation}
Notice that the MO coefficient derivatives do not appear in the final HF gradient expression.

They disappear through the magic of Wigner's \(2n + 1\) rule. From page 25:
\begin{quote}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
where the nucleus-nucleus repulsion energy is included for completeness. This is final expression for the molecular Hessian derived in \eqref{eq:hooke_derivation_3}.
