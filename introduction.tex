...
\subsection{\texorpdfstring{\href{https://chemistry.stackexchange.com/q/74683/194}{Series expansion}}{Series expansion}}
...
TODO mu is component of dipole (moment), alpha is component of polarizability, beta is component of first hyperpolarizability, gamma is component of second hyperpolarizability, etc., each describing an addition correction to how a system interacts with the external electric field.
...
\begin{equation}
  ...
  E(\mathbf{P}, \mathbf{Q}) = E^{(0,0)} + \left(E^{(1,0)} + E^{(0,1)}\right) + \left(E^{(2,0)} + E^{(0,2)} + E^{(1,1)}\right) + \dots,
\end{equation}
...
\begin{equation}
  ...
  1 + 1 = 2,
\end{equation}
where the term is directly related to the optical rotation, given as \(\braket{\braket{\hat{\mu};\hat{m}}}_{\omega}\) in table~\ref{tab:norman}. Section~\ref{TODO} will show how frequency dependence can be properly introduced to this term and in general.

TODO find optical rotation/quadratic response article from Filipp Furche
...
\begin{align}
  ...
  \braket{k|\psi_{i}^{(1)}} &= \frac{\braket{k|\hat{V}|\psi_{i}^{(0)}}}{\left( E_{i}^{(0)} - E_{k}^{(0)} \right)},
\end{align}
...
\begin{equation}
  ...
  \ket{\psi_{i}^{(1)}} = \sum_{k\neq i} \ket{k} \frac{\braket{k|\hat{V}|\psi_{i}^{(0)}}}{\left( E_{i}^{(0)} - E_{k}^{(0)} \right)}.
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}
    E_{i}^{(2)} &= \bra{\psi_{i}^{(0)}} \hat{V} \left( \sum_{k\neq i} \ket{k} \frac{\braket{k|\hat{V}|\psi_{i}^{(0)}}}{\left( E_{i}^{(0)} - E_{k}^{(0)} \right)} \right) \\
    &= \sum_{k \neq i} \frac{\braket{\psi_{i}^{(0)}|\hat{V}|k}\braket{k|\hat{V}|\psi_{i}^{(0)}}}{\left( E_{i}^{(0)} - E_{k}^{(0)} \right)}.
  \end{aligned}
\end{equation}
